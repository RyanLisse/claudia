...
const ELECTRIC_API_URL = process.env.ELECTRIC_URL || 'https://api.electric-sql.cloud';
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!ELECTRIC_SOURCE_ID || !ELECTRIC_SECRET) {
      console.error('Electric Cloud credentials not configured');
      return NextResponse.json(
    ...
    if (!response.ok) {
      ...
      console.error('Electric API error:', response.status, errorText);
      return NextResponse.json(
