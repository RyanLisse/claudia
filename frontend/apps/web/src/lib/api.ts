...
// Initialize invoke function based on environment
const initializeInvoke = async () => {
	if (typeof window !== "undefined" && "__TAURI__" in window) {
		...
		// Web deployment - provide fallback or API endpoint calls
		invoke = async (command: string, args?: any) => {
			console.warn(`API not available in web deployment: ${command}`, args);
			// For web deployment, you might want to call actual API endpoints
			// For now, return mock data or throw appropriate errors
			throw new Error(`API command "${command}" not available in web deployment`);
		};
	}
};

// Initialize invoke on module load
initializeInvoke();
 ...
 */
const ELECTRIC_URL =
	process.env.NEXT_PUBLIC_ELECTRIC_URL || "https://api.electric-sql.com";
const SOURCE_ID = process.env.NEXT_PUBLIC_ELECTRIC_SOURCE_ID;

/**
 * API client for interacting with Tauri backend and Electric SQL
 */
export const api = {
	/**
	...
	/**
	 * Electric SQL utilities
	 */
	electric: {
		/**
		 * Get Electric SQL shape URL with authentication
		 */
		getShapeUrl: (table: string, params?: Record<string, string>) => {
		...
		/**
		 * Get Electric SQL configuration for useShape hook
		 */
		getShapeConfig: (table: string, params?: Record<string, string>) => {
			return {
