...
export const ELECTRIC_CONFIG = {
  ...
  baseUrl: process.env.NEXT_PUBLIC_ELECTRIC_URL || 'https://api.electric-sql.cloud',
 ...
 */
export function getElectricShapeUrl(table: string, params?: Record<string, string>) {
  const url = new URL(`${ELECTRIC_CONFIG.baseUrl}/${ELECTRIC_CONFIG.apiVersion}/shape`);
