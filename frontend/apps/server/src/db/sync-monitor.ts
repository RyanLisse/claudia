...
export class SyncMonitor {
  ...
  async startMonitoring(intervalMs: number = 5000): Promise<void> {
    ...
    console.log(`✅ Sync monitoring started (${intervalMs}ms interval)`);
  }
  ...
  private notifySubscribers(status: SyncStatus): void {
    this.subscribers.forEach(callback => {
      try {
        ...
        console.error('Error notifying sync status subscriber:', error);
      }
    });
  }
  ...
  async getSyncStatus(): Promise<SyncStatus> {
    ...
    const latency = latencyMetrics.length > 0 
    ...
    const bandwidth = bandwidthMetrics.length > 0 
      ? bandwidthMetrics.reduce((sum, m) => sum + m.value, 0) / bandwidthMetrics.length 
      : 0;
  ...
  async getSyncHealth(): Promise<SyncHealth> {
    ...
    if (!status.isOnline) {
      ...
      recommendations.push('Check network connectivity and ElectricSQL service');
    }

    // Check pending events
    if (status.pendingEvents > 100) {
      score -= 20;
      issues.push(`High number of pending sync events (${status.pendingEvents})`);
      recommendations.push('Consider optimizing sync batch size or frequency');
    } else if (status.pendingEvents > 50) {
      score -= 10;
      issues.push(`Moderate number of pending sync events (${status.pendingEvents})`);
    }
    ...
    } else if (status.latency > 2000) { // 2 seconds
      score -= 10;
      issues.push(`Elevated sync latency (${Math.round(status.latency)}ms)`);
    }

    // Check bandwidth usage
    if (status.bandwidth > 1000000) { // 1MB
      score -= 10;
      issues.push(`High bandwidth usage (${Math.round(status.bandwidth / 1024)}KB)`);
  ...
  async getDetailedMetrics(hours: number = 24): Promise<any> {
  ...
  async cleanupOldData(days: number = 30): Promise<void> {
    ...
    await db.delete(syncEvents).where(
      and(
    ...
    await db.delete(syncMetrics).where(
      gte(syncMetrics.measuredAt, cutoff)
    );

    console.log(`🧹 Cleaned up sync data older than ${days} days`);
  }
}
