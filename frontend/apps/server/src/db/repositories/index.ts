...
import { users, projects, agents, sessions, messages, memory } from '../schema';
import { User, NewUser, Project, NewProject, Agent, NewAgent, Session, NewSession, Message, NewMessage, Memory, NewMemory } from '../schema';
...
export class AgentRepository extends BaseRepository<Agent, NewAgent> {
  ...
  async updateLastRun(id: string, success: boolean, runTime: number): Promise<void> {
    const agent = await this.findById(id);
    if (!agent) return;

    const totalRuns = agent.totalRuns + 1;
    const successfulRuns = success ? agent.successfulRuns + 1 : agent.successfulRuns;
  ...
  async updateMetrics(id: string, metrics: Partial<Agent['metrics']>): Promise<Agent> {
    const agent = await this.findById(id);
...
export class SessionRepository extends BaseRepository<Session, NewSession> {
  ...
  async findRecent(userId?: string, days: number = 7): Promise<Session[]> {
  ...
  async updateTokenUsage(id: string, tokens: number, cost: number): Promise<void> {
...
export class MessageRepository extends BaseRepository<Message, NewMessage> {
  ...
  async getMessageThread(sessionId: string, parentMessageId?: string): Promise<Message[]> {
    if (parentMessageId) {
...
export class MemoryRepository extends BaseRepository<Memory, NewMemory> {
  ...
  async findByTags(tags: string[], userId?: string): Promise<Memory[]> {
    ...
    return this.findMany(conditions.length > 0 ? and(...conditions) : undefined);
  }

  async findByImportance(minImportance: number, userId?: string): Promise<Memory[]> {
