...
import { setupTestDatabase, cleanupTestDatabase, clearAllTables, testDataFactory, getTestDb } from './setup';
...
describe('Database Repositories', () => {
  ...
  describe('UserRepository', () => {
    ...
    it('should find active users', async () => {
      await userRepository.create(testDataFactory.createUser({ isActive: true }));
      await userRepository.create(testDataFactory.createUser({ isActive: false }));
  ...
  describe('ProjectRepository', () => {
    ...
    it('should find projects by owner', async () => {
      const project1 = await projectRepository.create(testDataFactory.createProject(testUser.id));
      const project2 = await projectRepository.create(testDataFactory.createProject(testUser.id));
      
      const otherUser = await userRepository.create(testDataFactory.createUser());
      await projectRepository.create(testDataFactory.createProject(otherUser.id));
    ...
    it('should search projects', async () => {
      await projectRepository.create(testDataFactory.createProject(testUser.id, {
        name: 'React App',
    ...
    it('should increment session count', async () => {
      const project = await projectRepository.create(testDataFactory.createProject(testUser.id));
  ...
  describe('AgentRepository', () => {
    ...
    beforeEach(async () => {
      testUser = await userRepository.create(testDataFactory.createUser());
      testProject = await projectRepository.create(testDataFactory.createProject(testUser.id));
    });
    ...
    it('should find agents by type', async () => {
      await agentRepository.create(testDataFactory.createAgent(testUser.id, { type: 'coder' }));
      await agentRepository.create(testDataFactory.createAgent(testUser.id, { type: 'coder' }));
      await agentRepository.create(testDataFactory.createAgent(testUser.id, { type: 'researcher' }));
    ...
    it('should update agent run statistics', async () => {
      const agent = await agentRepository.create(testDataFactory.createAgent(testUser.id));
    ...
    it('should update agent metrics', async () => {
      ...
      const updatedAgent = await agentRepository.updateMetrics(agent.id, newMetrics);
  ...
  describe('SessionRepository', () => {
    ...
    beforeEach(async () => {
      testUser = await userRepository.create(testDataFactory.createUser());
      testProject = await projectRepository.create(testDataFactory.createProject(testUser.id));
      testAgent = await agentRepository.create(testDataFactory.createAgent(testUser.id));
    });
    ...
    it('should find sessions by user', async () => {
      const session1 = await sessionRepository.create(testDataFactory.createSession(testUser.id));
      const session2 = await sessionRepository.create(testDataFactory.createSession(testUser.id));
      
      const otherUser = await userRepository.create(testDataFactory.createUser());
      await sessionRepository.create(testDataFactory.createSession(otherUser.id));
    ...
    it('should update token usage', async () => {
      const session = await sessionRepository.create(testDataFactory.createSession(testUser.id));
  ...
  describe('MessageRepository', () => {
    ...
    beforeEach(async () => {
      testUser = await userRepository.create(testDataFactory.createUser());
      testSession = await sessionRepository.create(testDataFactory.createSession(testUser.id));
    });
    ...
    it('should find messages by session', async () => {
      const message1 = await messageRepository.create(testDataFactory.createMessage(testSession.id, 1));
      const message2 = await messageRepository.create(testDataFactory.createMessage(testSession.id, 2));
      
      const otherUser = await userRepository.create(testDataFactory.createUser());
      const otherSession = await sessionRepository.create(testDataFactory.createSession(otherUser.id));
      await messageRepository.create(testDataFactory.createMessage(otherSession.id, 1));

      const sessionMessages = await messageRepository.findBySession(testSession.id);
    ...
    it('should get session statistics', async () => {
      await messageRepository.create(testDataFactory.createMessage(testSession.id, 1, {
        role: 'user',
        tokens: 100,
        cost: 5
      }));
      
      await messageRepository.create(testDataFactory.createMessage(testSession.id, 2, {
        role: 'assistant',
        tokens: 200,
        cost: 10
      }));
      
      await messageRepository.create(testDataFactory.createMessage(testSession.id, 3, {
        role: 'user',
        tokens: 50,
        cost: 3
      }));
  ...
  describe('MemoryRepository', () => {
    ...
    it('should find memory by key', async () => {
      const memory = await memoryRepository.create(testDataFactory.createMemory({
        key: 'unique_key_123',
        userId: testUser.id
      }));
    ...
    it('should find memory by type and user', async () => {
      await memoryRepository.create(testDataFactory.createMemory({
        type: 'user_preference',
        userId: testUser.id
      }));
      ...
      const preferences = await memoryRepository.findByType('user_preference', testUser.id);
    ...
    it('should update memory access tracking', async () => {
      ...
      expect(updatedMemory?.lastAccessedAt?.getTime()).toBeGreaterThan(originalAccessTime?.getTime() || 0);
    });
