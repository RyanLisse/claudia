...
const TEST_DATABASE_URL = process.env.TEST_DATABASE_URL || 'postgresql://test:test@localhost:5432/claudia_test';
...
export const testDataFactory = {
  ...
  createProject: (ownerId: string, overrides: Partial<schema.NewProject> = {}): schema.NewProject => ({
  ...
  createAgent: (createdBy: string, overrides: Partial<schema.NewAgent> = {}): schema.NewAgent => ({
  ...
  createSession: (userId: string, overrides: Partial<schema.NewSession> = {}): schema.NewSession => ({
    title: `Test Session ${Date.now()}`,
    userId,
    ...overrides
  }),

  createMessage: (sessionId: string, sequenceNumber: number, overrides: Partial<schema.NewMessage> = {}): schema.NewMessage => ({
    sessionId,
    role: 'user',
  ...
  createMemory: (overrides: Partial<schema.NewMemory> = {}): schema.NewMemory => ({
...
export async function waitFor(condition: () => Promise<boolean>, timeoutMs: number = 5000): Promise<void> {
...
export function createMockSyncEvent(tableName: string, recordId: string, operation: 'insert' | 'update' | 'delete') {
  return {
    eventType: operation,
    tableName,
    recordId,
