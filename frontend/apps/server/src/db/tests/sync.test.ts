...
describe("Sync System", () => {
	...
	describe("SyncMonitor", () => {
		...
		it("should get sync status", async () => {
			...
			await db
				.insert(syncEvents)
				.values(createMockSyncEvent("users", user.id, "insert"));
		...
		it("should calculate sync health", async () => {
			...
			expect(health.issues.some((issue) => issue.includes("latency"))).toBe(
				true,
			);
		});
	...
	describe("ConflictResolver", () => {
		...
		it("should auto-resolve based on conflict type", async () => {
			...
			const result = await ConflictResolver.autoResolve(
